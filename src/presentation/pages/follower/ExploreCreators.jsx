...
export const ExploreCreators = () => {
    ...
    if (isLoading) {
        return <div className="text-center py-20 text-flansly-flan font-mono animate-pulse">Cocinando catálogo de creadores...</div>;
    }

    if (error) {
        return <div className="text-center py-20 text-flansly-error font-['Inter']">⚠️ {error}</div>;
    }

    return (
        <div className="space-y-6">
            <div>
                <h2 className="text-3xl font-extrabold text-[#F9F9F9] font-['Manrope'] tracking-tight">Explorar Creadores</h2>
                    ...
                    <div key={creator.id} className="relative bg-flansly-card border border-flansly-surface/30 rounded-2xl overflow-hidden group flex flex-col justify-between shadow-lg">
                        ...
                        <div className="w-full h-24 bg-flansly-surface overflow-hidden relative">
                            {creator.bannerImageUrl && (
                                <img src={creator.bannerImageUrl} alt="Banner" className="w-full h-full object-cover" />
                            )}
                            ...
                            >
                                <Star size={16} fill={creator.isFavorite ? "#FDE68A" : "none"} className={creator.isFavorite ? "text-flansly-flan" : ""} />
                            </button>
                        </div>
                        ...
                        <div className="p-5 flex flex-col items-center -mt-10 relative z-10 flex-1">
                            <img 
                                src={creator.profileImageUrl || '/flansly_logo.png'} 
                                alt={creator.displayName} 
                                className="w-16 h-16 rounded-full border-2 border-flansly-card object-cover bg-flansly-surface shadow-md"
                            />
                            <h4 className="text-base font-bold text-[#F9F9F9] font-['Manrope'] mt-3 truncate w-full text-center">
                                {creator.displayName}
                            </h4>
